...
its named locations on land, writing:
  ...
  src/data/realms/<id>.js          the map definition (points in image pixels)
...
Aqshy already uses real art (see src/data/realms/aqshy.js), so it isn't here.
...
def write_def(realm_id, title, points, named):
    ...
    path = os.path.join(ROOT, 'src', 'data', 'realms', f'{realm_id}.js')
...
def main():
    ...
    os.makedirs(os.path.join(ROOT, 'src', 'data', 'realms'), exist_ok=True)
