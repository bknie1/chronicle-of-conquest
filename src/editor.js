...
function loadMap(id) {
  ...
  // Realm maps have no factions of their own; their setting (src/data/mortal-realms.js) owns homes.
  state.ownsFactions = Array.isArray(src.factions) && src.factions.length > 0;
  state.map.factions ||= [];
  state.isNew = !MAPS[id];
  state.selected = null;
  state.mode = 'select';
...
function renderPanel() {
  ...
  panel.innerHTML = `
    ...
    <ul id="faction-homes">${m.factions.map((f, i) => `
      ...
      </li>`).join('') || `<li class="muted">${MAPS[m.id] && !state.ownsFactions
        ? "This map's faction homes and realmgates are set by its setting in src/data/. Points used there can't be deleted or renamed here."
        : 'No factions yet.'}</li>`}</ul>`;
...
async function save() {
  ...
  try {
    const res = await fetch(`/api/dev/maps/${encodeURIComponent(state.mapId)}`, {
      ...
      body: JSON.stringify(state.ownsFactions || state.map.factions.length ? state.map : { ...state.map, factions: undefined }),
    });
    ...
    updateUndoButton();
    if (state.isNew) {
      const varName = state.mapId.replace(/-([a-z0-9])/g, (_, c) => c.toUpperCase());
      openModal(`
        <h2>Saved</h2>
        <p>Wrote <code>${esc(data.wrote)}</code>. This map isn't registered yet. Add it to <code>src/data/maps/index.js</code>:</p>
        <pre style="white-space:pre-wrap;background:var(--panel-2);padding:8px;border-radius:5px;font-size:13px">import ${esc(varName)} from './${esc(state.mapId)}.js';

// ...and add ${esc(varName)} to the list passed to Object.fromEntries.</pre>
        <p class="muted">To play on it, add it to a setting in <code>src/data/settings.js</code>.</p>
